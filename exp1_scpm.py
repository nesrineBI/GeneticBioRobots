...
config = None
results_file = 'result/exp3/results_scpm_exp3.p'
export_img = 'img/ga/scpm_exp3'
...
for i in range(0, channels):
 ...
 if model_conf[i] is not None:
   ...
   scpm.load_conf(model_conf[i])
            
 scpm.set_dt(time)
 scpms_list.update({i: scpm})
old_stn = [0.0 for _ in range(0, channels)]


for channel in range(0, channels):
	thresholds.update({channel: scpms_list[channel].get_theta_gpi()})
...
for s in range(0, nb_sim):
	...
	for dt in range(0, int(1/time)):
		...
		for c in range(channels):
			values = scpms_list[c].compute_d1_to_gpi(salience[c][s], old_stn)
...
for o in outputs:
    if o in [0, 1, 2]:
        ...
        ax = plt.gca()
        ax.set_title('scpm' + ": channel " + str(o + 1))
        ax.set_xlim([0, 5])
        ax.set_ylim([0, 1])
